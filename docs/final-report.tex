...
To address the observed ranking inconsistencies and improve operational readiness, we propose:
\begin{enumerate}
    \item \textbf{CVSS normalization layer}: Add a dedicated agent to detect version mismatches and convert all scores to V3.1 using published conversion heuristics before synthesis, ensuring fair comparison with traditional scoring.
    \item \textbf{Automated benchmark suite}: Script \texttt{src/eval/} with NVD ground-truth CVEs, compute Mean Absolute Error (MAE) and ranking correlation (Kendall's $\tau$) across $n \geq 50$ samples to quantify systematic deviations from traditional CVSS.
    \item \textbf{Validator agent}: Introduce a post-synthesis step that cross-checks generated scores against retrieved CVSS base metrics, flagging discrepancies for human review—bridging the gap between automated retrieval and manual precision.
    \item \textbf{Hybrid scoring mode}: Allow the system to invoke traditional CVSS calculators as a fallback when SEPSES data is incomplete, combining the reliability of established methods with agentic enrichment.
    \item \textbf{User study}: Deploy the system with security analysts to evaluate whether the structured explanations and multi-source evidence justify the accuracy trade-offs compared to traditional CVSS workflows.
\end{enumerate}
